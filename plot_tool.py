...
def plot_spectrum(rec_spec, true_spec, path, sr, hop_length):
...
def plot_spectrums(rec_spec_batch, true_spec_batch, out_folder, sr, hop_length):
    for i in range(rec_spec_batch.shape[0]):
        path = out_folder + 'rec_vs_original_spectrum_sample_' + str(i) + '.png'
        plot_spectrum(rec_spec_batch[i], true_spec_batch[i], path, sr, hop_length)
